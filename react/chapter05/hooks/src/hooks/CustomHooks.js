...
/* 
  첫 번째 렌더링시 뉴스 피드를 구독할 수 있다.
  그 후 정리 함수에서 뉴스 피드 구독을 취소할 수 있다..
  첫 번째 렌더링시 posts라는 상태 변수를 만들고 이변수의 상태를 변경하는 setPosts라는 함수를 만들 수 있다. 그 후 addPosts라는 함수를 만들어서 최신 뉴스를 얻어서 배열에 추가한다.
  그 후 useEffect를 사용해 뉴스 피드를 구독하고, 벨을 울린다.
  그리고 정리 함수를 반환한다. 이 함수는 이별의 벨소리를 울리고 뉴스 구독을 취소한다.
*/
