...
export default function DependencyRelation() {
  ...
  /* 
  렌더링이 이뤄질 때마다 효과가 호출되는 것을 바라지는 않는다. useEffect 훅을 구체적인 데이터 변경과 연동시킬 필요가 있다. 
  ...
  */
  ...
  /* 
    useEffect의 두 번째 인자로 빈 배열을 넘길 수도 있다.
    빈 의존 관계 배열은 초기 렌더링 직후 이펙트가 단 한번만 호출되게 한다.
  ...
  */
    ...
    useEffect를 컴포넌트의 생성과 제거에 사용할 수 있다는 뜻이다.
    빈 배열은 최초 렌더링시 환영한다는 벨소리가 한번만 플레이 된다는 뜻이다. 그 후에는 컴포넌트가 트리에서 제거될 때 이별의 벨소리를 울리는 함수를 반환한다.
    */
    const welcomeChimePlay=()=>{
      console.log("환영의 벨소리가 울린다.")
    }
